...
void DefaultBestPositionCallback(Position best_position, double time_stamp) {
  std::cout << "BESTPOS: \nGPS Week: " << best_position.header.gps_week <<
            "  GPS milliseconds: "
            << best_position.header.gps_millisecs << std::endl <<
            "  Latitude: "
            << best_position.latitude << std::endl <<
            "  Longitude: "
            << best_position.longitude << std::endl <<
            "  Height: "
            << best_position.height << std::endl << std::endl;
}
...
bool Novatel::Connect_(std::string port, int baudrate = 115200) {
  try {
    ...
    if (!Ping()) {
      std::stringstream output;
      output << "Novatel GPS not found on port: " << port << " at baudrate "
             << baudrate << std::endl;
      log_error_(output.str());
      delete serial_port_;
      serial_port_ = NULL;
...
void Novatel::BufferIncomingData(unsigned char *message, unsigned int length)
  ...
  for (unsigned int ii = 0; ii < length; ii++) {
    ...
    if (buffer_index_ == SYNC_1_IDX) {	// looking for beginning of message
      ...
      data_buffer_[buffer_index_++] = message[ii];
      bytes_remaining_--;
      message_id = BINARY_LOG_TYPE(
          ((data_buffer_[MSG_ID_END_IDX]) << 8)
              + data_buffer_[MSG_ID_END_IDX - 1]);
      ...
      bytes_remaining_ = (header_length_ - (MSG_LENGTH_END_IDX + 1))
          + CHECKSUM_SIZE + (data_buffer_[MSG_LENGTH_END_IDX] << 8)
          + data_buffer_[MSG_LENGTH_END_IDX - 1];
  ...
  switch (message_id) {
    ...
    case INSPVAS_LOG_TYPE:
      ...
      if (ins_position_velocity_attitude_short_callback_)
        ins_position_velocity_attitude_short_callback_(ins_pva_short,
                                                       read_timestamp_);
      break;
    ...
    case PSRDOPB_LOG_TYPE:
      ...
      memcpy(&psr_dop.prn, message + header_length + 28,
             (4 * psr_dop.number_of_prns));
      //Copy CRC
    ...
    case SATXYZB_LOG_TYPE:
      ...
      //Copy repeated fields
      memcpy(&sat_pos.data, message + header_length + 12,
             (68 * sat_pos.number_of_satellites));
      //Copy CRC
      memcpy(&ranges.crc, message + header_length + payload_length, 4);
    ...
    case SATVISB_LOG_TYPE:
      ...
      memcpy(&sat_pos, message, header_length + 12);
      //Copy repeated fields
      memcpy(&sat_vis.data, message + header_length + 12,
             (40 * sat_vis.number_of_satellites));
      //Copy CRC
      memcpy(&ranges.crc, message + header_length + payload_length, 4);
    ...
    case TRACKSTATB_LOG_TYPE:
      ...
      //Copy repeated fields
      memcpy(&tracking_status.data, message + header_length + 16,
             (40 * tracking_status.number_of_channels));
      //Copy CRC
