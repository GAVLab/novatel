                              ...
                              double &timestamp)
{
  std::cout << "[" << std::setprecision(2) << std::fixed << timestamp <<
              ...
              range.number_of_observations << std::endl;
    ...
    double &timestamp)
{
  ...
  for (size_t kk = 0; kk < range.number_of_observations; ++kk)
  {
    std::cout << "\tPRN: " << range.range_data[kk].range_record.satellite_prn <<
        "\tC/N0: " << range.range_data[kk].range_record.carrier_to_noise <<
        "\tPSR: " << range.range_data[kk].range_record.pseudorange << std::endl;
  }
...
void PositionEcefHandler(novatel::PositionEcef &pos, double &timestamp)
{
  std::cout << "[" << std::setprecision(2) << std::fixed << timestamp <<
              ...
              "   Type: " << pos.position_type << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "Buffer(" << sizeof(buff) << "):" << std::endl;
